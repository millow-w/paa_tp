...
def resolver_branch_and_bound(W, V, itens, timeout_seconds=None):
    """
    Resolve o problema da mochila usando branch and bound otimizado.
    """
    def _executar():
        bnb.melhor_valor = 0
        bnb.melhor_solucao = []
        ...
        for i in range(n):
            p, v, val = itens[i]
            lista_itens.append((p, v, val, i))
        
        lista_itens.sort(key=lambda x: x[2] / (x[0] + x[1]) if (x[0] + x[1]) > 0 else 0, reverse=True)
        
        valor_inicial, sol_inicial = bnb.calcular_solucao_gulosa_inicial(n, W, V, lista_itens)
        bnb.melhor_valor = valor_inicial
        bnb.melhor_solucao = sol_inicial
        
        vetor = [False] * n
        inicio = time.time()
        
        bnb.backtrack(vetor, 0, n, W, V, lista_itens, 0, 0, 0)
...
def main():
    ...
    # Instâncias de teste
    instancias = [
        "../instancias/W20_V20_variando_N/instancia_n10.txt",
